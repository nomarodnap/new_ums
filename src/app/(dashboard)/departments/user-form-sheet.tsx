...
export function UserFormSheet({ user, department, open, onOpenChange }: { user: any, department: any, open: boolean, onOpenChange: (open: boolean) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const data = {
      ...
      departmentId: department.id, // Lock to this department
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="overflow-y-auto sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <label htmlFor="role" className="text-sm font-medium">สิทธิ์การใช้งาน (Role)</label>
            <select 
              id="role" 
              name="role" 
