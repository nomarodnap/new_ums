...
import { updateUserAdmin, sendUserResetPassword } from "@/server/actions/users";
...
export function DepartmentUsersSheet({
  ...
  const toggleBan = async (user: UserData) => {
    if (
      confirm(
        `คุณต้องการ ${user.banned ? "ปลดระงับ" : "ระงับ"} บัญชี ${user.name} หรือไม่?`,
      )
    ) {
  ...
  const handleSendResetEmail = async (email: string) => {
    ...
    try {
      const res = await sendUserResetPassword(email);
      if (res?.success) {
        alert(`ระบบได้ส่งลิงก์สำหรับตั้งรหัสผ่านไปยัง ${email} เรียบร้อยแล้ว`);
      } else {
        alert(res?.error || "ไม่สามารถส่งอีเมลได้ กรุณาลองใหม่อีกครั้ง");
      }
    } catch (e: any) {
      alert(e.message || "เกิดข้อผิดพลาดในการส่งอีเมล");
    } finally {
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1 items-start">
                        {user.banned ? (
                          <Badge
                            variant="secondary"
                            className="bg-destructive/10 text-destructive border-destructive/20 hover:bg-destructive/20 font-medium"
                          >
                            ถูกระงับ
                          </Badge>
                        ) : !user.password ? (
                          <Badge
                            variant="outline"
                            className="bg-amber-50 text-amber-700 border-amber-300 dark:bg-amber-950/60 dark:text-amber-400 dark:border-amber-700 font-medium"
                          >
                            รอยืนยันอีเมล์
                          </Badge>
                        ) : (
