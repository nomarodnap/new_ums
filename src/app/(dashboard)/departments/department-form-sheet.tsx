    ...
    <Sheet open={open} onOpenChange={setOpen}>
      {trigger
        ...
        : !isControlled && (
            <SheetTrigger
              render={
                <Button>
                  <PlusCircle className="mr-2 h-4 w-4" /> เพิ่มหน่วยงาน
                </Button>
              }
            />
          )}
      <SheetContent className="overflow-y-auto sm:max-w-xl">
        <SheetHeader>
          ...
          <SheetDescription>กรอกข้อมูลรายละเอียดของหน่วยงาน</SheetDescription>
        </SheetHeader>

        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <label htmlFor="costCenterCode" className="text-sm font-medium">
                รหัสศูนย์ต้นทุน
              </label>
              <Input
                id="costCenterCode"
                name="costCenterCode"
                defaultValue={department?.costCenterCode || ""}
              />
            </div>
            <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input
              id="fullName"
              name="fullName"
              defaultValue={department?.fullName || ""}
              required
            />
            {errors?.fullName && (
              <span className="text-xs text-red-500">{errors.fullName[0]}</span>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                id="province"
                name="province"
                defaultValue={department?.province || ""}
              />
            </div>
          </div>

          <div className="flex flex-col gap-2">
            <label htmlFor="depositUnit" className="text-sm font-medium">
              รหัสหน่วยรับฝาก
            </label>
            <DepartmentCombobox
