  ...
  return (
    <div className="flex-1 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">รายงานการบริหารงบประมาณ</h2>
          <p className="text-muted-foreground mt-1">เปรียบเทียบงบประมาณจัดสรร งบใช้ไป และงบคงเหลือรายหน่วยงาน</p>
        </div>
      ...
      <div className="rounded-md border bg-card print:border-none print:shadow-none">
        <Table>
          ...
          <TableBody>
            {reportData.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center h-24 text-muted-foreground">ไม่มีข้อมูลการตั้งงบประมาณ</TableCell>
              </TableRow>
            )}
            {reportData.map((row, idx) => {
              ...
              return (
                <TableRow key={idx} className={isWarning ? "bg-red-50/50" : ""}>
                  ...
                  <TableCell className="text-right">{allocated.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</TableCell>
                  <TableCell className="text-right">{used.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</TableCell>
                  <TableCell className={`text-right font-bold ${remaining < 0 ? 'text-red-600' : ''}`}>
